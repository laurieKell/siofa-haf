---
title: "**SIOFA Sharks**"
title: "Data Analysis of SIOFA Deep Sea Sharks"
author: "Mariana"
...
This document provides a summary of the shark dataset, which includes information 
on shark captures such as species, length, weight, sex, maturity, and other relevant 
data. The aim is to understand the diversity of species, the types of data collected, 
and assess the quality and coverage of the dataset.
...
```{r data-preparation}
# Load data
load("../data/inputs/Sharks_biological.Rdata") #biol
load("../data/inputs/Sharks_logbooks.Rdata") #cpue

# Standardize sex variable:
table(biol$sex, useNA = "always")

# Filtering out rows where 'sex' is "3", "2", "4", "5", or "H"
biol <- biol %>%
  filter(!sex %in% c("3", "2", "4", "5", "H"))
...
table(biol$mat, useNA = "always")

# Ensuring 'mat' is a character before replacement
biol$mat <- as.character(biol$mat)  

biol$mat[biol$mat %in% c("A", "A/1", "F1", "M1", "N1")] <- "1"
biol$mat[biol$mat %in% c("B", "B/2", "F2", "M2")]       <- "2"
biol$mat[biol$mat %in% c("C", "C/3", "F3", "M3")]       <- "3"
biol$mat[biol$mat %in% c("D", "D/4", "F4", "M4")]       <- "4"
biol$mat[biol$mat %in% c("E/5", "F5")]                  <- "5"
biol$mat[biol$mat %in% c("F/6", "F6")]                  <- "6"
biol$mat[biol$mat %in% c("G", "F7", "G/7")]             <- "7"
biol$mat[biol$mat=="F8"]  <- "8"

# Convert 'mat' to numeric
biol$mat <- as.numeric(biol$mat)
...
summarized_catch <- cpue %>%
  filter(!is.na(y3) & !is.na(x3) & !is.na(year)) %>%
  group_by(N_Cientifico, Arte, year, x3, y3) %>%
  summarize(TotalCatch = sum(Kilos, na.rm = TRUE))

# Rename year
summarized_catch <- rename(summarized_catch, Year = year)
...
unique_gears <- unique(summarized_catch$Arte)
unique_species <- unique(summarized_catch$N_Cientifico)
...
for (year in unique_years) {
  for (gear in unique_gears) {
    ...
    year_gear_data <- subset(summarized_catch, Year == year & Arte == gear)
    # Check if the year_gear_data is not empty
    if (nrow(year_gear_data) > 0) {
      # Create the plot with adjusted settings
      p <- ggplot() +
        world_map +
        geom_point(data = year_gear_data, aes(x = x3, y = y3, size = TotalCatch, color = N_Cientifico), alpha = 0.8) +
...
avg_catch_weight_by_species <- cpue %>%
  group_by(N_Cientifico) %>%
  summarize(AverageCatchWeight = mean(Kilos, na.rm = TRUE))

# Create bar plot
ggplot(avg_catch_weight_by_species, aes(x = reorder(N_Cientifico, AverageCatchWeight), y = AverageCatchWeight)) +
...
catch_by_species <- cpue %>%
  group_by(N_Cientifico) %>%
  summarize(TotalCatch = sum(Kilos, na.rm = TRUE)) %>%
  arrange(desc(TotalCatch))

# Bar chart of catch composition by species
ggplot(catch_by_species, aes(x = reorder(N_Cientifico, TotalCatch), y = TotalCatch)) +
...
species_count_by_area <- cpue %>%
  group_by(Area, N_Cientifico) %>%
  summarize(Count = n(), .groups = 'drop')

# Grouped bar chart
ggplot(species_count_by_area, aes(x = Area, y = Count, fill = N_Cientifico)) +
...
```{r species-temporal-trend}

# Aggregate by year and species
species_yearly_trend <- cpue %>%
  group_by(year, N_Cientifico) %>%
  summarize(TotalCatch = sum(Kilos, na.rm = TRUE))

# Line graph for species-specific yearly trends
ggplot(species_yearly_trend, aes(x = year, y = TotalCatch, color = N_Cientifico)) +
  geom_line() +
...
ggplot(subset(biol, !is.na(Length)), aes(x = Length)) +
...
avg_length_by_species <- biol %>%
  group_by(Scientific_name) %>%
  summarize(AverageLength = mean(Length, na.rm = TRUE))

# Plot the average lenghts 
ggplot(avg_length_by_species, aes(x = reorder(Scientific_name, AverageLength), y = AverageLength)) +
  geom_bar(stat = "identity", fill = "blue") +
...
load("../data/lh.RData")
lh=transform(lh,Scientific_name=paste(Genus,Species,sep=" "))
lh=subset(lh,Scientific_name%in%unique(biol$Scientific_name))

# Bar plot on the fish sizes distributions for each species 
ggplot(subset(biol, !is.na(Length)), aes(x = Length)) +
  geom_histogram(bins = 30, fill = "blue", color = "black") +
  geom_vline(aes(xintercept=exp(Lm)), data=lh, col="green")+
  geom_vline(aes(xintercept=exp(Loo)),data=lh, col="black")+
  facet_wrap(~Scientific_name, scales = "free")+
  theme_minimal() +
  theme(plot.background = element_rect(fill = "white", colour = "white"),
        panel.background = element_rect(fill = "white", colour = "white")) +
  labs(title = "Distribution of Fish Sizes by Species, with L50 and Loo", x = "Length", y = "Count")
```
...
ggplot(subset(biol, !is.na(sex) & !is.na(Length)), aes(x = Length, fill = sex)) +
...
ggplot(subset(biol, !is.na(MaturityDescription) & !is.na(Length)), aes(x = MaturityDescription, y = Length, fill = MaturityDescription)) +
  geom_boxplot() +
...
ggplot(gender_ratio_by_species, aes(x = Scientific_name, y = Count, fill = Gender)) +
...
maturity_count_by_species <- biol %>%
  filter(!is.na(MaturityDescription), !is.na(sex)) %>%
  group_by(Scientific_name, MaturityDescription, sex) %>%
  summarize(Count = n(), .groups = 'drop')

# Bar plot showing the proportion of each maturity stage within each species.
ggplot(maturity_count_by_species, aes(x = Scientific_name, y = Count, fill = MaturityDescription)) +
